...
pub const Brain = struct
{
    ...
    pub fn update(self: *Brain, fireTruster: fn(self: *bb.Bug, id: usize)void, bself: *bb.Bug)void
    {
        ...
        for(hids, 0..hids.len)|hid, i|
        {
            ...
            hids[i].varsum = hids[i].varsum / hids.len;
        ...
        for(outs, 0..outs.len)|out, i|
        {
            ...
            outs[i].varsum = outs[i].varsum / outs.len;
    ...
    pub fn mutate(self: *Brain)void
    {
        ...
        for(hidden, 0..hidden.len)|nuron, i|
        {
           ...
           for(cons, 0..cons.len)|con,ii|
           {
                ...
                if(b)cons[ii].weight += a*rate;
                if(!b)cons[ii].weight -= a*rate;
           }
           const a = rand.float(f32);
           const b = rand.boolean();

           if(b)hidden[i].thresold += a*rate;
           if(!b and hidden[i].thresold > 0)hidden[i].thresold -= a*rate;
        }
        ...
        for(outputs, 0..outputs.len)|nuron, i|
        {
           ...
           const a = rand.float(f32);
           const b = rand.boolean();
           if(b and outputs[i].thresold<0.9)outputs[i].thresold += a*rate;
           if(!b and outputs[i].thresold>0)outputs[i].thresold -= a*rate;
        }
        ...
        for(inputs, 0..inputs.len)|nuron, i|
        {
           ...
           for(cons, 0..cons.len)|con,ii|
           {
            ...
            if(b and inputs[i].thresold < 1) inputs[i].thresold += a*rate;
            if(!b and inputs[i].thresold > 0) inputs[i].thresold -= a*rate;
